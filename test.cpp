...
bool DynamicStackTest::test4() {
    ...
    stack1.push(300);
    ASSERT_TRUE(stack1.peek() == 300)

    // Check the state of the capacities before popping.
    ASSERT_TRUE(stack1.init_capacity_ == 5 && stack1.capacity_ == 5 && !stack1.empty() && stack1.size_ == 3)

    // Peek at the top element, then remove it.
    ASSERT_TRUE(stack1.peek() == 300 && stack1.pop() == 300)
    ASSERT_TRUE(stack1.peek() == 200 && stack1.pop() == 200)
    ASSERT_TRUE(stack1.peek() == 100 && stack1.pop() == 100)
...
bool DynamicStackTest::test5() {
    ...
    ASSERT_TRUE(stack1.init_capacity_ == 5 && stack1.capacity_ == 10)

    for(int i = 0; i < 5; i++){
        stack1.push(i);
    }
    ASSERT_TRUE(stack1.init_capacity_ == 5 && stack1.capacity_ == 20)
    return true;
}
...
bool DynamicStackTest::test7() {
    ...
    ASSERT_TRUE(stack.capacity_ == 100);
    ASSERT_TRUE(stack.size_ == 75);
    for (int i = 0; i < 49; i++) {
        stack.pop();
    }
    ASSERT_TRUE(stack.size() == 26);
    ASSERT_TRUE(stack.peek() == 26)
    ASSERT_TRUE(stack.capacity_ == 100);
    stack.pop();
    ASSERT_TRUE(stack.capacity_ == 50);
    ASSERT_TRUE(stack.peek() == 25)
    stack.pop();
    ASSERT_TRUE(stack.peek() == 24)
    stack.pop();
    ASSERT_TRUE(stack.peek() == 23)

    DynamicStack stack2(10);
    for (int i = 0; i < 10; i++) {
        stack2.push(i);
    }
    for (int i = 0; i < 8; i++) {
    ...
    ASSERT_TRUE(stack2.capacity_ == 10);
    stack2.pop();
    stack2.pop();
    ASSERT_TRUE(stack2.pop() == DynamicStack::EMPTY_STACK);
    return true;
}
...
bool DynamicStackTest::test9() {
    ...
    ASSERT_TRUE(stack.capacity_ == 14)
